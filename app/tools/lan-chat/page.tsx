...
export default function LanChatPage() {
  ...
  const react = async (message: Message, emoji: string) => { try { const next = await request<Message>(`/api/chat/messages/${message.id}/reactions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ emoji }) }); setMessages((current) => current.map((item) => item.id === next.id ? next : item)); } catch (reason) { setError(reason instanceof Error ? reason.message : '表情发送失败。'); } };
  ...
  return <main className={`lan-chat min-h-screen bg-[#edf3ef] text-slate-800 ${darkTheme ? 'lan-chat-dark' : ''}`}><div className="mx-auto flex min-h-screen max-w-7xl overflow-hidden bg-[#f7faf8] shadow-2xl shadow-emerald-950/10 lg:min-h-[100vh]">
    ...
    <section className="flex min-h-screen min-w-0 flex-1 flex-col"><header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 sm:px-6"><div><h1 className="text-base font-semibold">局域网大厅</h1><p className="mt-0.5 text-xs text-slate-400">{online.length ? `${online.length} 人在线` : '正在连接…'} · 全部历史已保存</p></div><div className="flex items-center gap-2"><button type="button" onClick={() => setDarkTheme((current) => !current)} title={darkTheme ? '切换为浅色主题' : '切换为深色主题'} className="rounded-xl border border-slate-200 bg-slate-100 px-2.5 py-2 text-xs font-medium text-slate-600 transition hover:bg-slate-200">{darkTheme ? '☀ 浅色' : '◐ 深色'}</button><div className="relative"><input value={search} onChange={(event) => void searchMessages(event.target.value)} placeholder="搜索聊天记录" className="w-32 rounded-xl bg-slate-100 px-3 py-2 text-xs outline-none transition focus:w-48 focus:ring-2 focus:ring-emerald-200 sm:w-48" />{search && <button type="button" onClick={() => { setSearch(''); setSearchResults(null); }} className="absolute right-2 top-1.5 text-sm text-slate-400">×</button>}</div><span className="grid h-8 w-8 place-items-center rounded-full bg-emerald-600 text-xs font-bold text-white lg:hidden">{user?.username.slice(0, 1).toUpperCase()}</span></div></header>
      ...
      <div ref={messageAreaRef} onScroll={(event) => { if (event.currentTarget.scrollTop < 60) void loadOlder(); }} className={`relative flex-1 overflow-y-auto px-3 py-5 sm:px-6 ${dragging ? 'bg-emerald-50' : ''}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={(event) => { if (event.currentTarget === event.target) setDragging(false); }} onDrop={onDrop}>
        ...
        <div className="mx-auto max-w-3xl space-y-5">{visibleMessages.length === 0 ? <div className="py-24 text-center"><div className="text-4xl">👋</div><p className="mt-4 font-medium text-slate-600">还没有消息</p><p className="mt-1 text-sm text-slate-400">发出第一句问候，开启局域网大厅。</p></div> : visibleMessages.map((message) => { const own = message.author.username === user?.username; return <article key={message.id} className={`group flex gap-2.5 ${own ? 'flex-row-reverse' : ''}`}><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-full text-sm font-semibold ${own ? 'bg-emerald-600 text-white' : 'bg-orange-100 text-orange-700'}`}>{message.author.username.slice(0, 1).toUpperCase()}</span><div className={`relative min-w-0 max-w-[82%] sm:max-w-[72%] ${own ? 'items-end' : ''}`}><div className={`mb-1 flex items-center gap-2 text-[11px] text-slate-400 ${own ? 'justify-end' : ''}`}><span>{message.author.username}</span><span>{time(message.createdAt)}</span></div>{message.withdrawnAt ? <div className="rounded-2xl bg-slate-200 px-3 py-2 text-sm italic text-slate-500">这条消息已被撤回</div> : <><div className={`rounded-2xl px-3.5 py-2.5 shadow-sm ${own ? 'rounded-tr-md bg-emerald-500 text-white' : 'rounded-tl-md bg-white text-slate-800'}`}>{message.replyTo && <button type="button" onClick={() => setReplyTo(message.replyTo as Message)} className={`mb-2 block max-w-full rounded-lg px-2 py-1 text-left text-xs ${own ? 'bg-emerald-600/40 text-emerald-50' : 'bg-slate-100 text-slate-500'}`}>回复 @{message.replyTo.author.username}：{message.replyTo.withdrawnAt ? '已撤回' : (message.replyTo.body || '[附件]').slice(0, 46)}</button>}{message.body && <MessageText body={message.body} />}{message.attachments.map((attachment) => <AttachmentView key={attachment.id} attachment={attachment} />)}</div><div className={`chat-message-tools pointer-events-none absolute z-10 mt-1 flex flex-nowrap items-center gap-1 opacity-0 transition-all duration-150 ${own ? 'right-0 top-full translate-y-1' : 'left-0 top-full translate-y-1'} group-hover:pointer-events-auto group-hover:translate-y-0 group-hover:opacity-100`}>{message.reactions.map((reaction) => <button type="button" key={reaction.emoji} onClick={() => void react(message, reaction.emoji)} className={`rounded-full border px-2 py-0.5 text-xs ${reaction.reacted ? 'border-emerald-300 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-white text-slate-600'}`}>{reaction.emoji} {reaction.count}</button>)}<div className="hidden gap-1 group-hover:flex">{QUICK_EMOJIS.map((emoji) => <button type="button" key={emoji} onClick={() => void react(message, emoji)} className="rounded-full bg-white px-1.5 py-0.5 text-xs shadow-sm">{emoji}</button>)}</div><button type="button" onClick={() => setReplyTo(message)} className="hidden rounded-full bg-white px-2 py-0.5 text-xs text-slate-500 shadow-sm group-hover:block">回复</button>{own && isFresh(message.createdAt) && <button type="button" onClick={() => void withdraw(message)} className="hidden rounded-full bg-white px-2 py-0.5 text-xs text-slate-500 shadow-sm group-hover:block">撤回</button>}{user?.isAdmin && <button type="button" onClick={() => void removeAsAdmin(message)} className="hidden rounded-full bg-rose-50 px-2 py-0.5 text-xs text-rose-600 group-hover:block">删除</button>}</div></>}</div></article>; })}</div></div>
