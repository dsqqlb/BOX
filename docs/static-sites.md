...
把站点域名的 DNS 指向同一台服务器（或同一个隧道）即可，不需要额外的 Web 服务器。

用主站域名的另一个子域可以吗？可以，而且推荐：主站 `www.example.com` + 站点 `box.example.com`。
会话 Cookie 不带 `Domain` 属性，只属于主站那个主机名，不会发到站点域名，隔离效果与完全不同的域名一致。要求只有两个：

- 反向代理**保留原始 `Host` 头**，否则进程无法按 Host 分流（两个域名写进同一个 `server` 块一起反代即可）；
- `BOX_SITES_HOST` 与 `BOX_PRIMARY_HOST` **不能是同一个域名**：同一个域名会被静态站点接管，BOX 主站页面会全部变成 404。

实际配置示例（含 nginx）见[部署指南](./deployment.md)第 10.1 节。
...
- 访问地址：`<站点域名>/<站点名>/`（协议与反向代理一致：现在用 `http`，上了 HTTPS 就用 `https`）
...
- 站点域名与主站域名同属一个注册域（如 `box.` 与 `www.`）时，浏览器的 SameSite 不再提供额外隔离，
  写操作完全依赖同源校验：所有会改动 BOX 数据的接口都要求 `Origin` 与 `Host` 同源，站点域名里的 JS 调用它们只会拿到 403。
