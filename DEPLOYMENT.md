...
- **主屏幕/遥控器掉线、设备没电重开**（服务进程本身没重启）：数据不会丢。房间只要还有主屏幕或遥控器连着，就永远不会被自动清理；即使两边都断开了，也有1小时的宽限期。主屏幕在浏览器URL里没带房间号时（比如设备重开后是一个全新的页面）会先弹出"选择房间"界面，列出服务器上还活着的房间，点一下就能回到原来的战斗，不需要凭记忆重新输入房间号。这个列表由 `/api/rooms` 提供。

  > 安全提示：`/api/rooms` 会把当前所有活跃房间号列出来，等同于去掉了"需要知道6位房间号才能进入"这道门槛。这个项目本身就没有鉴权（房间号即权限），所以只适合内网/朋友间使用，不建议在完全公开的部署上使用这个接口。
