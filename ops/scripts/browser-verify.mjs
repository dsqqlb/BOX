...
// 用法：node ops/scripts/browser-verify.mjs  （需先 npm run dev / npm start 跑在 9999）
...
// 依赖统一装在 code/node_modules，脚本文件在 ops/scripts，所以要显式从 code/ 解析。
const WebSocket = createRequire(path.join(CODE_ROOT, 'package.json'))('ws');
...
const GLOBALS_CSS = fs.readFileSync(path.join(CODE_ROOT, 'app', 'globals.css'), 'utf8');
