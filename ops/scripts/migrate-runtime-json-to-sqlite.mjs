...
const savingsFile = path.join(dataDir, 'savings.json');
const dndSavesDir = path.join(dataDir, 'dnd', 'saves');
const backupDir = path.join(dataDir, 'backups', `runtime-json-to-sqlite-${new Date().toISOString().replace(/[:.]/g, '-')}`);
