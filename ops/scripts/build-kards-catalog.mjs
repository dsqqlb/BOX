#!/usr/bin/env node
/**
 * 扫描 resources/public/image/Kards 的卡图目录结构，生成 resources/content/kards/cards.json 目录。
 *
 * 目录约定（与资源本身的组织方式一致）：
...
const kardsRoot = path.join(projectRoot, 'resources', 'public', 'image', 'Kards');
const outFile = path.join(projectRoot, 'resources', 'content', 'kards', 'cards.json');
