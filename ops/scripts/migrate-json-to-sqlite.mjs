 ...
 * 原始 JSON 会复制到 resources/data/backups/json-to-sqlite-<timestamp>/，不会删除。
 */
...
const { prisma } = require('../../code/server/db');
const root = path.resolve(import.meta.dirname, '..', '..');
const dataDir = path.join(root, 'resources', 'data');
const usersPath = path.resolve(process.env.BOX_AUTH_USERS_FILE || path.join(dataDir, 'auth-users.json'));
const decksDir = path.join(dataDir, 'edh', 'decks');
const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
const backupDir = path.join(dataDir, 'backups', `json-to-sqlite-${timestamp}`);
