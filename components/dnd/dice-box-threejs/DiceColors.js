...
export class DiceColors {
	...
	constructor(options = {}) {
		...
		this._textureDataCache = {}
	}

	// 按纹理名单独解析出一份"纹理数据"（含已加载好的贴图/凹凸图），供"每种骰子形状单独指定纹理"功能使用。
	// 跟getColorSet()不同：这个不依赖颜色方案，纯粹只是"给我某个纹理名对应的可用纹理对象"，
	// 使用纹理自带的默认材质(见const/texturelist.js里每个纹理定义的material字段)，
	// 内部做了缓存，同一纹理被多个形状复用时不会重复加载图片。
	async getTextureData(texturename) {
		if (this._textureDataCache[texturename]) return this._textureDataCache[texturename]

		let tex = this.getTexture(texturename)
		tex = await this.ImageLoader(tex)

		this._textureDataCache[texturename] = tex
		return tex
	}
