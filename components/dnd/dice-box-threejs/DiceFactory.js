...
class DiceFactory {
	...
	create(type) {
		...
		// 按形状（d4/d6/d8/d10/d12/d20）单独指定纹理：如果为这个形状配置了专属纹理，
		// 用它覆盖掉setMaterialInfo()刚算出来的"整体颜色方案纹理"，颜色本身(dice_color/label_color等)不受影响。
		// 注意用diceobj.shape而不是type去查——d100在这个库里shape是'd10'，所以d100会自动跟随d10的纹理设置，
		// 这正是"同一种形状统一用一种纹理"的语义（d100物理上就是一个d10）。
		if (this.shapeTextureOverrides && this.shapeTextureOverrides[diceobj.shape]) {
			this.dice_texture_rand = this.shapeTextureOverrides[diceobj.shape];
		}
