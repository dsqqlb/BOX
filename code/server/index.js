 ...
 *   1. 页面：生产环境托管 next build 导出的静态产物(code/out)，开发环境挂 Next.js dev server（含HMR热更新）
 *      图片、字体等资源类文件统一放在 resources/public，两种模式都由静态文件模块回退提供
...
async function main() {
  ...
  if (config.DEV) {
    ...
    // Next.js 的工程根是 code/（next.config.ts、app/ 都在这里），资源类目录在它之外。
    const app = next({ dev: true, dir: config.CODE_ROOT });
    await app.prepare();
