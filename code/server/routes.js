...
function createRequestHandler({ auth, userData, edhDecks, carcassonneSaves, accountAdmin, homePreferences, medicineStore, sceneMedia, holdemStore, siteStore, siteHosting, roomServer, kardsRoomServer, chatServer, holdemRoomServer, config, adminTracking }) {
  ...
  return async function requestHandler(req, res, ctx) {
    ...
    // 一切都按「账号 + 存档栏位（0/1/2）」隔离，首页切换栏位就是换一整套数据。
    if (pathname === '/api/notequest/slots' || pathname.startsWith('/api/notequest/slots/')) {
      try {
        const rest = pathname.slice('/api/notequest/slots'.length).replace(/^\/+/, '');
        if (!rest) {
          if (req.method !== 'GET') return httpUtils.sendAuthError(res, 405, '只支持 GET。');
          return httpUtils.sendJson(res, { slots: await noteQuestStore.listSlots(requestUser.username), slotCount: noteQuestStore.SLOT_COUNT });
        }
        const target = noteQuestStore.normalizeSlot(rest.split('/')[0]);
        ...
        const cleared = await noteQuestStore.clearSlot(requestUser.username, target);
        return httpUtils.sendJson(res, { success: true, cleared });
      } catch (error) {
    ...
    if (pathname === '/api/notequest/runs' || pathname.startsWith('/api/notequest/runs/') || pathname === '/api/notequest/graves') {
      ...
      try {
        if (pathname === '/api/notequest/graves') {
          if (req.method !== 'GET') return httpUtils.sendAuthError(res, 405, '只支持 GET。');
          return httpUtils.sendJson(res, { graves: await noteQuestStore.listGraves(requestUser.username, noteQuestStore.normalizeSlot(slotParam)) });
        }

        const rest = pathname.slice('/api/notequest/runs'.length).replace(/^\/+/, '');
        if (!rest) {
          if (req.method === 'GET') {
            return httpUtils.sendJson(res, { runs: await noteQuestStore.listRuns(requestUser.username, noteQuestStore.normalizeSlot(slotParam)) });
          }
          if (req.method === 'POST') {
            if (!httpUtils.isSameOrigin(req)) return httpUtils.sendAuthError(res, 403, '缺乏同源验证。');
            ...
            return httpUtils.sendJson(res, { run: await noteQuestStore.createRun(requestUser.username, body, body.slot ?? slotParam) }, 201);
          }
          return httpUtils.sendAuthError(res, 405, '不支持的请求方法。');
        }
    ...
    // NoteQuest 人物池：掷骰建角与自定义建角都写这里，进地牢时随便挑一个带走。
    if (pathname === '/api/notequest/characters' || pathname.startsWith('/api/notequest/characters/')) {
      const slotParam = requestUrl.searchParams.get('slot');
      try {
        const rest = pathname.slice('/api/notequest/characters'.length).replace(/^\/+/, '');
        if (!rest) {
          if (req.method === 'GET') {
            return httpUtils.sendJson(res, { characters: await noteQuestStore.listCharacters(requestUser.username, noteQuestStore.normalizeSlot(slotParam)) });
          }
          if (req.method === 'POST') {
            if (!httpUtils.isSameOrigin(req)) return httpUtils.sendAuthError(res, 403, '缺乏同源验证。');
            const raw = await httpUtils.readRawBody(req, noteQuestStore.MAX_BODY_BYTES);
            if (raw === null) return httpUtils.sendAuthError(res, 413, '角色数据过大或请求体不合法。');
            let body = null;
            try { body = JSON.parse(raw); } catch { body = null; }
            if (!body || typeof body !== 'object') return httpUtils.sendAuthError(res, 400, '缺乏请求体。');
            return httpUtils.sendJson(res, { character: await noteQuestStore.createCharacter(requestUser.username, body, body.slot ?? slotParam) }, 201);
          }
          return httpUtils.sendAuthError(res, 405, '不支持的请求方法。');
        }

        const characterId = rest.split('/')[0];
        if (req.method === 'GET') {
          const character = await noteQuestStore.getCharacter(requestUser.username, characterId);
          return character ? httpUtils.sendJson(res, { character }) : httpUtils.sendAuthError(res, 404, '角色不存在。');
        }
        if (req.method === 'PATCH') {
          if (!httpUtils.isSameOrigin(req)) return httpUtils.sendAuthError(res, 403, '缺乏同源验证。');
          const raw = await httpUtils.readRawBody(req, noteQuestStore.MAX_BODY_BYTES);
          ...
          try { body = JSON.parse(raw); } catch { body = null; }
          if (!body || typeof body !== 'object') return httpUtils.sendAuthError(res, 400, '缺乏请求体。');
          const character = await noteQuestStore.updateCharacter(requestUser.username, characterId, body);
          return character ? httpUtils.sendJson(res, { character }) : httpUtils.sendAuthError(res, 404, '角色不存在。');
        }
        if (req.method === 'DELETE') {
          if (!httpUtils.isSameOrigin(req)) return httpUtils.sendAuthError(res, 403, '缺乏同源验证。');
          if (!(await noteQuestStore.deleteCharacter(requestUser.username, characterId))) return httpUtils.sendAuthError(res, 404, '角色不存在。');
          return httpUtils.sendJson(res, { success: true });
        }
        return httpUtils.sendAuthError(res, 405, '不支持的请求方法。');
      } catch (error) {
    ...
    // NoteQuest 永久地牢：一个账号 × 一种地牢类型 = 一张永久地图（含遗体与掉落）。
    if (pathname === '/api/notequest/dungeons' || pathname.startsWith('/api/notequest/dungeons/')) {
      const slotParam = requestUrl.searchParams.get('slot');
      try {
        const rest = pathname.slice('/api/notequest/dungeons'.length).replace(/^\/+/, '');
        if (!rest) {
          if (req.method === 'GET') {
            // ?typeId=palace&slot=1：按地牢类型取这个栏位里那张永久地图（新一局复用同一张图）
            const byType = String(requestUrl.searchParams.get('typeId') || '').trim();
            if (byType) {
              const dungeon = await noteQuestStore.getDungeonByType(requestUser.username, byType.slice(0, 40), noteQuestStore.normalizeSlot(slotParam));
              return httpUtils.sendJson(res, { dungeon });
            }
            return httpUtils.sendJson(res, { dungeons: await noteQuestStore.listDungeons(requestUser.username, noteQuestStore.normalizeSlot(slotParam)) });
          }
          if (req.method === 'PUT') {
            if (!httpUtils.isSameOrigin(req)) return httpUtils.sendAuthError(res, 403, '缺乏同源验证。');
            const raw = await httpUtils.readRawBody(req, noteQuestStore.MAX_DUNGEON_BYTES + 64 * 1024);
            if (raw === null) return httpUtils.sendAuthError(res, 413, '地牢地图过大或请求体不合法。');
            let body = null;
            try { body = JSON.parse(raw); } catch { body = null; }
            if (!body || typeof body !== 'object') return httpUtils.sendAuthError(res, 400, '缺乏请求体。');
            return httpUtils.sendJson(res, { dungeon: await noteQuestStore.saveDungeon(requestUser.username, body, body.slot ?? slotParam) });
          }
          return httpUtils.sendAuthError(res, 405, '不支持的请求方法。');
        }

        const dungeonId = rest.split('/')[0];
        if (req.method === 'GET') {
          const dungeon = await noteQuestStore.getDungeon(requestUser.username, dungeonId);
          return dungeon ? httpUtils.sendJson(res, { dungeon }) : httpUtils.sendAuthError(res, 404, '地牢不存在。');
        }
        if (req.method === 'DELETE') {
          if (!httpUtils.isSameOrigin(req)) return httpUtils.sendAuthError(res, 403, '缺乏同源验证。');
          if (!(await noteQuestStore.deleteDungeon(requestUser.username, dungeonId))) return httpUtils.sendAuthError(res, 404, '地牢不存在。');
          return httpUtils.sendJson(res, { success: true });
        }
        return httpUtils.sendAuthError(res, 405, '不支持的请求方法。');
      } catch (error) {
...
// 家庭药箱：库存供具有 medicine-inventory 权限的家庭账户共享；图片只通过本受保护端点读取。
    ...
    // 页面与静态资源：
    // - /dnd/* 是独立打包的纯静态应用（code/dnd-app），先于其它静态根匹配，URL 保持 /dnd/ 不变；
    // - 生产：读 code/out 构建产物，找不到时回退 resources/public（图片、字体、子素材等）；
    // - 开发：resources/public 里的资源由本项目直接提供，其余页面与 /_next 交给 Next dev server（保留 HMR）。
    //   Next 只认工程目录下的 public/，而资源类目录已经移出 code/，所以开发模式也要先查一次资源目录。
    const dndAsset = staticFiles.resolveWithinDndApp(pathname);
    if (dndAsset) return staticFiles.sendStaticFile(req, res, dndAsset, 200, staticFiles.cacheControlFor(pathname));

    const found = staticFiles.resolveStaticFile(pathname);
    if (found) return staticFiles.sendStaticFile(req, res, found, 200, staticFiles.cacheControlFor(pathname));
    if (config.DEV) return ctx.nextRequestHandler(req, res);
    return staticFiles.sendNotFound(req, res);
  };
}
