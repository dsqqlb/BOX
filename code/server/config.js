...
// 目录布局（单一事实来源，其他模块一律从这里取路径）：
//   <PROJECT_ROOT>/code/       代码类：页面、组件、服务端、构建产物
//   <PROJECT_ROOT>/resources/  资源类：.env.local、data、public、content
//   <PROJECT_ROOT>/docs/       文档类
//   <PROJECT_ROOT>/ops/        命令行类：脚本、发布、检查、日志
// data / public 与代码分离，是为了部署时能整目录上传下载，而不必碰代码。
...
const CONTENT_DIR = path.join(RESOURCES_DIR, 'content');
const ENV_FILE = path.join(RESOURCES_DIR, '.env.local');
...
const CHAT_MAX_UPLOAD_BYTES = Number.isFinite(configuredChatUploadBytes)
  ...
  : 1073741824;
const MEDICINE_DIR = path.join(DATA_DIR, 'medicine');
...
const MEDICINE_MAX_UPLOAD_BYTES = Number.isFinite(configuredMedicineUploadBytes)
  ...
  : 10 * 1024 * 1024;
const INITIATIVE_SCENE_DIR = path.join(DATA_DIR, 'initiative-scenes');
