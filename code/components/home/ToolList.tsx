...
export default function ToolList({ tools, favorites = new Set(), onToggleFavorite, onToolOpen }: ToolListProps) {
  return <div className="home-tool-list overflow-hidden rounded-2xl border border-white/[0.09] bg-[#0a1320]/60">{tools.map((tool, index) => <article key={tool.slug} className={`group relative flex items-center gap-4 px-4 py-4 transition hover:bg-white/[0.045] sm:px-5 ${index > 0 ? 'border-t border-white/[0.07]' : ''}`}>
