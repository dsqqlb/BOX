...
export default function ToolGrid({ tools, viewMode, favorites, onToggleFavorite, onToolOpen, emptyMessage = '没有找到匹配的工具' }: ToolGridProps) {
  ...
  return <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{tools.map((tool) => <ToolCard key={tool.slug} tool={tool} isFavorite={favorites?.has(tool.slug)} onToggleFavorite={onToggleFavorite} onToolOpen={onToolOpen} />)}</div>;
}
